...
from resources.data import RAID, REPLYRAID, BGTSPAM
...
async def spam(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(Bgt) == 2:
            ...
            g = a.id
            if int(g) in Bgt:
                text = f"I can't raid on @Bgt_Chat's Owner"
...
async def _(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(e.text) > 11:
            ...
            if int(user_id) in Bgt:
                text = f" can't raid on @bgt_chat's Owner."
                await e.reply(text, parse_mode=None, link_preview=None )
                ...
                phucker = [user_id]
                gey.append(phucker)
                text = f" Bgt Activated replyraid"
                await e.reply(text, parse_mode=None, link_preview=None )
        elif e.reply_to_msg_id:             
            a = await e.get_reply_message()
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(e.text) > 12:
            ...
            try:
                ...
                pass
            text = " Bgt De-Activated Reply Raid"
            await e.reply(text, parse_mode=None, link_preview=None )
        elif e.reply_to_msg_id:             
            a = await e.get_reply_message()
            b = await e.client.get_entity(a.sender_id)
...
async def _(event):
   ...
   if event.sender_id in SUDO_USERS:
         if event.text[0].isalpha() and e.text[0] in ("/", "#", "@", "!"):
            return await e.reply(usage, parse_mode=None, link_preview=None )
         Bgt = "".join(event.text.split(maxsplit=1)[1:]).split(" ", 2)
         if len(Bgt) == 3:
             user = str(Bgt[2])
             ...
             if int(e) in BGTSPAM:
                ...
                await event.reply(text, parse_mode=None, link_preview=None )
                 ...
                 counter = int(Bgt[1])
                 sleeptimet = sleeptimem = float(Bgt[0])
                 for _ in range(counter):
                      reply = random.choice(RAID)
                      caption = f"{username} {reply}"
                      async with event.client.action(event.chat_id, "typing"):
